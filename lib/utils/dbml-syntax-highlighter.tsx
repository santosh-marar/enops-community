function DBMLHighlighter({ code }: { code: string }) {
  ...
  const tokenize = (line: string) => {
    ...
    while (remaining.length > 0) {
      ...
      // keywords
      const keyword = remaining.match(/^(Table|Ref|Enum|indexes|ref|pk|null|not null|unique|default|note|as|headercolor)\b/i);
      if (keyword) {
      ...
      // types
      const type = remaining.match(/^(int|integer|varchar|text|boolean|bool|uuid|timestamp|date|time|float|decimal|bigint|serial|json|jsonb|enum)\b/i);
      if (type) {
  ...
  return (
    <pre
      style={{ background: "#27272a", borderRadius: "0.5rem", padding: "1rem", overflowX: "auto", fontSize: "0.8rem", lineHeight: "1.6" }}
    >
      <code>
        {lines.map((line, i) => (
          <div key={i}>
